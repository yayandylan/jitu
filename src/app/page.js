      ...
      <div className="py-8 bg-white border-y border-slate-100 overflow-hidden relative z-20">
        <div className="max-w-7xl mx-auto px-6 text-center mb-6">
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-slate-400 flex items-center justify-center gap-2">
                <Cpu size={12} className="text-blue-500"/> Powered By World Class AI Engine
            </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 auto-rows-[260px]">
            <div className="md:col-span-2 row-span-1 md:row-span-2 bg-[#0F172A] rounded-[2rem] p-10 relative overflow-hidden group shadow-2xl text-white flex flex-col justify-between border border-slate-800 hover:border-blue-900 transition-colors">
                ...
                <div className="relative z-10">
                    ...
                    <p className="text-slate-400 text-sm font-medium leading-relaxed max-w-sm">Jangan bakar uang untuk produk yang tidak diinginkan pasar. Biarkan AI membedah potensi cuan sebelum Anda mulai spend iklan sepeserpun.</p>
                </div>
            </div>
            ...
            <div className="md:col-span-2 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[2rem] p-8 relative overflow-hidden group shadow-xl text-white flex items-center justify-between border border-blue-500/50">
                 ...
                 <div className="h-24 w-24 bg-white/10 rounded-full flex items-center justify-center backdrop-blur-md border border-white/20 group-hover:rotate-12 transition-transform duration-500 shadow-2xl"><Layers size={40} /></div>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
            <div className="space-y-6">
              ...
              <p className="text-slate-400 text-xs font-medium leading-relaxed uppercase tracking-wide opacity-70">AI Marketing Intelligence Platform No.1 di Indonesia. Membantu UMKM & Agency scaling tanpa boncos.</p>
          ...
          <div className="pt-8 border-t border-slate-800 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-6">
                <div className="flex items-center gap-2"><Globe size={12} className="text-slate-500"/><span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Indonesia (ID)</span></div>
                <div className="flex items-center gap-2 px-3 py-1 bg-slate-900 rounded-full border border-slate-800"><div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" /><span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Systems Normal</span></div>
            </div>
          </div>
        </div>
      </footer>
...
function PricePack({ title, price, totalPoints, highlight, bonus, goldTheme, badge }) {
  return (
    <div className={`p-8 rounded-[2.5rem] border transition-all text-left relative overflow-hidden h-fit flex flex-col group ${highlight ? 'bg-[#0F172A] border-slate-900 shadow-2xl md:scale-110 z-10 text-white' : goldTheme ? 'bg-white border-amber-200 shadow-xl shadow-amber-500/10' : 'bg-white border-slate-100 hover:border-blue-200 hover:shadow-lg'}`}>
      ...
      <div className={`space-y-5 ${badge ? 'mt-4' : ''}`}>
        ...
        <div className={`py-4 px-6 rounded-2xl font-black text-2xl uppercase tracking-tighter flex items-center justify-between border ${highlight ? 'bg-blue-600 border-blue-500 text-white' : goldTheme ? 'bg-amber-50 border-amber-100 text-amber-600' : 'bg-slate-50 border-slate-100 text-slate-700'}`}>
        ...
        <Link href="/register" className={`mt-8 block w-full py-4 rounded-xl font-black text-[10px] uppercase tracking-[0.3em] transition-all text-center active:scale-95 border hover:shadow-lg ${highlight ? 'bg-white text-slate-900 border-white hover:bg-blue-50' : goldTheme ? 'bg-amber-500 text-white border-amber-500 hover:bg-amber-600' : 'bg-slate-900 text-white border-slate-900 hover:bg-blue-600 hover:border-blue-600'}`}>BELI SEKARANG</Link>
      </div>
    </div>
  );
}