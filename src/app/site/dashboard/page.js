...
export default function DashboardPage() {
  ...
  // Data Statis (Placeholder untuk konten yang belum ada di DB)
  const trendingTools = [
    ...
    <div className="max-w-[1280px] mx-auto space-y-6 pb-24 pt-4 font-sans antialiased text-slate-900 px-4 md:px-6">
      ...
      <div className="flex justify-between items-center bg-white/70 backdrop-blur-md p-4 md:p-5 rounded-[2rem] border border-slate-100 shadow-sm">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 md:w-12 md:h-12 bg-slate-900 rounded-2xl flex items-center justify-center text-white shadow-lg shrink-0 relative overflow-hidden">
            <Zap size={18} fill="currentColor" className="text-yellow-400 relative z-10" />
            {userData.isPremium && <div className="absolute inset-0 bg-gradient-to-tr from-amber-400/20 to-transparent"></div>}
          </div>
        ...
        <div className="lg:col-span-6 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-xl shadow-blue-500/20 group">
          ...
          <div className="relative z-10 space-y-6">
            ...
            <div className="space-y-2">
              <h2 className="text-2xl md:text-3xl font-black italic uppercase tracking-tighter leading-tight">Micro-SaaS: E-Course Automation</h2>
              <p className="text-blue-100 text-xs font-normal leading-relaxed italic opacity-80">"Niche edukasi automasi sedang meledak. Margin profit digital mencapai 90%."</p>
            </div>
            <Link href="/site/tools/riset-produk" className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest bg-white text-blue-600 px-8 py-3 rounded-xl hover:bg-blue-50 transition-all shadow-lg active:scale-95">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-5">
        ...
        {/* HISTORY AKTIVITAS */}
        <div className="lg:col-span-4 bg-white rounded-[2.5rem] p-7 border border-slate-100 shadow-sm space-y-6 flex flex-col">
          ...
          <div className="space-y-4 flex-1">
            {history.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center opacity-50 py-4">
                    ...
                    <p className="text-[10px] font-bold text-slate-400 uppercase">Belum ada aktivitas</p>
                </div>
            ) : (
                history.map((h, i) => (
        ...
        <div className="lg:col-span-5 bg-white rounded-[2.5rem] p-7 border border-slate-100 shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-1 gap-3">
            {articles.map((art) => (
              <Link href={`/site/academy?id=${art.id}`} key={art.id} className="p-4 bg-slate-50 rounded-3xl border border-transparent hover:border-blue-100 hover:bg-white transition-all group flex items-center justify-between shadow-sm active:scale-95">
                <div className="flex-1 min-w-0">
        ...
        {/* INSIGHT CARD */}
        <div className="lg:col-span-3 bg-emerald-50 rounded-[2.5rem] p-8 border border-emerald-100 flex flex-col justify-between relative overflow-hidden group shadow-sm">
          ...
          <Link href="/site/academy" className="w-full text-center py-3.5 bg-emerald-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest mt-8 hover:bg-slate-900 transition-all relative z-10 shadow-lg shadow-emerald-600/20 active:scale-95">
             Baca Strategi
          </Link>
        </div>
      </div>
