...
import { 
  CheckCircle, Trash2, Calendar, X, BarChart3, Banknote, Search, ArrowUpRight, ArrowDownLeft
} from 'lucide-react';
...
export default function AdminTransactions() {
  ...
  const handleUpdateStatus = async (id, status) => {
    if (!confirm(`Ubah status transaksi ini menjadi ${status.toUpperCase()}?`)) return;
    try {
  ...
  const handleDelete = async (id) => {
    if (!confirm("Hapus transaksi ini secara permanen? Data tidak bisa kembali.")) return;
    try {
  ...
  // --- LOGIKA STATISTIK (Realtime based on Filter) ---
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-20 px-4 mt-2 font-poppins antialiased text-slate-900">
      ...
      <div>
        ...
        <p className="text-slate-400 text-[10px] font-normal uppercase tracking-widest mt-1">Pantau arus kas masuk dan penggunaan API secara real-time</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-blue-600 p-6 rounded-[2rem] text-white shadow-lg shadow-blue-200">
          <div className="flex items-center gap-2 mb-2 opacity-80">
            ...
            <p className="text-[10px] font-bold uppercase tracking-wider">Omzet Topup</p>
          </div>
        ...
        <div className="bg-rose-600 p-6 rounded-[2rem] text-white shadow-lg shadow-rose-200">
           <div className="flex items-center gap-2 mb-2 opacity-80">
            <BarChart3 size={16} />
            <p className="text-[10px] font-bold uppercase tracking-wider">Modal API (Riil)</p>
          </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        {/* Input Pencarian */}
        <div className="flex-1 bg-white p-3 rounded-3xl border border-slate-100 flex items-center px-4 shadow-sm">
          ...
          <table className="w-full text-left">
            <thead className="bg-slate-50/50 border-b border-slate-100 font-bold text-slate-400 text-[9px] uppercase tracking-[0.2em]">
              <tr>
                ...
                <th className="px-6 py-6">{tab === 'in' ? 'Harga Jual' : 'Modal Riil'}</th>
                <th className="px-8 py-6 text-right">Aksi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {loading ? (
                 <tr><td colSpan="6" className="px-6 py-20 text-center text-slate-400 font-medium">Memuat data transaksi...</td></tr>
                ...
                searchedData.map((item) => (
                  <tr key={item._id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-5">
                      ...
                      <p className="text-[9px] text-slate-400 font-medium mt-0.5">{new Date(item.createdAt).toLocaleString('id-ID')}</p>
                    </td>
                    ...
                    <td className="px-6 py-5 font-bold text-slate-700">
                      {tab === 'in' ? (
                        <div className="space-y-1">
                             <span className="text-blue-700 font-black text-xs">Rp {(item.totalPrice || 0).toLocaleString('id-ID')}</span>
                             <div className={`text-[9px] font-bold uppercase tracking-wider w-fit px-1.5 py-0.5 rounded ${item.status === 'success' ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}>{item.status}</div>
                        </div>
                      ) : (
                    ...
                    <td className="px-8 py-5 text-right flex justify-end gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                      {item.status?.toLowerCase() === 'pending' && (
                        <button onClick={() => handleUpdateStatus(item._id, 'success')} title="Approve Manual" className="p-2 bg-emerald-50 text-emerald-600 rounded-xl hover:bg-emerald-500 hover:text-white transition-all"><CheckCircle className="w-4 h-4" /></button>
                      )}
                      <button onClick={() => handleDelete(item._id)} title="Hapus Permanen" className="p-2 bg-slate-50 text-slate-400 rounded-xl hover:bg-rose-500 hover:text-white transition-all"><Trash2 className="w-4 h-4" /></button>
                    </td>
                  </tr>
                ))
              )}
