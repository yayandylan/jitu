...
import { 
    ...
    History, ChevronLeft, X, Clock, Target, ShieldCheck 
} from 'lucide-react';
...
export default function ValidasiMarketPage() {
  ...
  // Default Pesan Awal (Persona Validator)
  const defaultChat = [
    { 
      role: 'assistant', 
      content: 'Halo! Saya **Jitu Validator**. 🎯\n\nJangan bakar uang dulu! Ceritakan produk apa yang mau Anda jual, saya akan cek apakah ada pasarnya atau tidak.\n\nSaya juga akan buatkan **Strategi Tes Iklan (CTWA)** termurah untuk membuktikan ide Anda winning atau boncos.' 
    }
  ];
  ...
  // Kalimat Loading ala Media Buyer
  const loadingMessages = [
    "Mengecek kepadatan kompetisi...",
    "Mencari angle iklan yang tajam...",
    "Menghitung estimasi biaya per lead...",
    "Merancang strategi CTWA...",
    "Memvalidasi rasa sakit pasar..."
  ];
  ...
  useEffect(() => {
    ...
    // Key LocalStorage BEDA dengan Riset Produk
    const savedChat = localStorage.getItem('JITU_CHAT_VALIDASI');
    const savedId = localStorage.getItem('JITU_ACTIVE_ID_VALIDASI');
  ...
  useEffect(() => {
    if (isLoaded.current) {
        if (chat.length > 1 || (chat.length === 1 && chat[0].content !== defaultChat[0].content)) {
            localStorage.setItem('JITU_CHAT_VALIDASI', JSON.stringify(chat));
        }
        if (activeHistoryId) {
            localStorage.setItem('JITU_ACTIVE_ID_VALIDASI', activeHistoryId);
        }
    }
    setTimeout(() => {
  ...
  const fetchConfig = async () => {
    try {
        ...
        if(confRes?.ok) {
            ...
            // Slug harus sesuai database: 'validasi-market'
  ...
  const handleSend = async (e) => {
    ...
    try {
      // Panggil API dengan tipe 'validasi-market'
  ...
  const handleReset = () => {
    if(confirm("Mulai validasi baru? Chat lama tersimpan di riwayat.")) {
        setChat(defaultChat);
        setActiveHistoryId(null);
        localStorage.removeItem('JITU_CHAT_VALIDASI');
        localStorage.removeItem('JITU_ACTIVE_ID_VALIDASI');
  ...
  const handleLoadHistory = (item) => {
    ...
    localStorage.setItem('JITU_ACTIVE_ID_VALIDASI', item._id);

    if (item.inputData?.chat) {
        setChat(item.inputData.chat);
        localStorage.setItem('JITU_CHAT_VALIDASI', JSON.stringify(item.inputData.chat));
    } else {
        const restored = [
            { role: 'user', content: `Produk: ${item.inputData.idea || '-'}` },
            { role: 'assistant', content: item.resultData }
        ];
        setChat(restored);
        localStorage.setItem('JITU_CHAT_VALIDASI', JSON.stringify(restored));
    }
    setIsHistoryOpen(false);
  };
      ...
      <div className="hidden lg:flex lg:col-span-3 flex-col gap-4 h-full overflow-hidden">
         <div className="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm shrink-0">
            <h1 className="text-sm font-black uppercase tracking-widest flex items-center gap-2 mb-2 text-blue-900">
                <Target className="w-4 h-4 text-blue-600" /> Market Validator
            </h1>
      ...
      <div className={`fixed top-0 right-0 bottom-0 w-[85%] max-w-sm bg-white z-50 shadow-2xl transition-transform duration-300 ease-in-out lg:hidden flex flex-col border-l border-slate-100 ${isHistoryOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
             <h3 className="text-xs font-black uppercase tracking-widest flex items-center gap-2 text-blue-900">
                <History size={16} className="text-blue-600" /> Riwayat Validasi
             </h3>
      ...
      <div className="lg:col-span-9 h-full flex flex-col bg-white rounded-xl md:rounded-[2.5rem] shadow-sm md:shadow-xl border border-slate-100 overflow-hidden relative">
        ...
        <div className="px-4 py-3 md:p-4 border-b border-slate-100 bg-white/80 backdrop-blur-md flex justify-between items-center shrink-0 sticky top-0 z-20">
            <div className="flex items-center gap-2 md:gap-3">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                        <h2 className="text-sm font-black uppercase tracking-tight text-slate-800 truncate">Jitu Validator</h2>
            ...
            <div className="flex items-center gap-2">
                <button onClick={() => setIsHistoryOpen(true)} className="lg:hidden p-2 bg-slate-50 text-blue-600 rounded-xl hover:bg-blue-50 transition-all border border-slate-200"><History size={18} /></button>
                ...
                <div className="flex justify-start w-full animate-in fade-in duration-500 px-1">
                    <div className="flex max-w-[90%] gap-3 items-end">
                        ...
                        <div className="bg-white px-4 py-3 rounded-2xl rounded-tl-sm border border-slate-200 shadow-sm">
                            <p className="text-xs font-bold text-blue-600 italic animate-pulse">{loadingMessages[loadingTextIndex]}</p>
                        </div>
                    </div>
                </div>
            )}
        ...
        <div className="p-3 md:p-5 bg-white border-t border-slate-100 shrink-0 sticky bottom-0 z-20">
            <form onSubmit={handleSend} className="relative flex items-end gap-2 max-w-4xl mx-auto">
                <div className="relative flex-1 bg-slate-50 border border-slate-200 rounded-[1.5rem] focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 transition-all flex items-center px-2">
                    <textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if(e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleSend(e); } }} placeholder="Validasi produk apa?" className="w-full bg-transparent border-none text-slate-800 text-sm font-medium px-3 py-3 md:py-4 outline-none placeholder:text-slate-400 resize-none max-h-[120px]" style={{ minHeight: '48px', height: input ? 'auto' : '48px' }} rows={1} disabled={loading || !config.isActive} />
                </div>
                <button type="submit" disabled={!input.trim() || loading || !config.isActive} className="w-12 h-12 md:w-14 md:h-14 bg-blue-600 text-white rounded-full hover:bg-blue-700 disabled:opacity-50 transition-all shadow-lg shadow-blue-500/30 active:scale-90 flex items-center justify-center shrink-0">
