...
export default function MagicAdScriptPage() {
  ...
  const fetchHistory = async () => {
    try {
      const res = await fetch('/api/history?tool=magic-ad-script');
  ...
  const handleAnalyze = async (e) => {
    ...
    try {
      ...
      // (Hapus bagian ini jika API AI Bapak sudah otomatis menyimpan history)
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-20 font-poppins antialiased text-slate-900">
      ...
      <div className="lg:col-span-2 space-y-8">
        ...
        <div className={`bg-white p-6 md:p-8 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 transition-all ${!config.isActive ? 'opacity-50 pointer-events-none grayscale' : ''}`}>
          <form onSubmit={handleAnalyze} className="space-y-6">
            ...
            <div>
                ...
                <input
                    type="text"
                    className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl text-sm font-medium outline-none focus:border-violet-500 focus:bg-white transition-all placeholder-slate-400"
            ...
            <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <textarea
                        className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl text-sm font-medium outline-none focus:border-violet-500 focus:bg-white transition-all h-32 resize-none placeholder-slate-400"
            ...
            <button
              ...
              className={`w-full text-white py-5 px-6 rounded-2xl font-black uppercase tracking-[0.2em] text-[11px] shadow-lg transition-all flex items-center justify-center gap-3 group active:scale-95 ${loading ? 'bg-slate-400' : 'bg-violet-600 hover:bg-violet-700 hover:shadow-violet-500/30'}`}
            >
              {loading ? (
                ...
                <>
                    ...
                    {config.isActive && (
                      <span className="bg-violet-800/40 text-[9px] font-bold py-1 px-2.5 rounded-lg text-violet-50 ml-1 border border-violet-400/30 normal-case tracking-normal">
        ...
        {/* --- HASIL GENERATE --- */}
        {result && (
          <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-violet-900/10 border border-slate-100 overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-700">
            {/* Header Hasil */}
              ...
              <article className="prose prose-sm max-w-none">
                <ReactMarkdown 
                    ...
                    components={{
                        // Judul Utama
                        h1: ({node, ...props}) => (
                            <h1 className="text-2xl md:text-3xl font-black text-slate-900 uppercase tracking-tighter mb-6 border-b-4 border-violet-500 pb-4 inline-block" {...props} />
                        ),
                        ...
                        h2: ({node, ...props}) => (
                            <div className="bg-violet-50 border border-violet-100 p-4 rounded-xl flex items-center gap-3 mt-10 mb-4 shadow-sm">
                                ...
                                <h2 className="text-sm font-black text-violet-900 uppercase tracking-widest m-0" {...props} />
                            </div>
                        ),
