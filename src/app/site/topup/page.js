...
export default function TopupPage() {
  ...
  useEffect(() => {
    const syncData = async () => {
        setFetching(true);
        try {
            // A. Ambil Harga Per Poin (GlobalSetting)
            ...
            console.error("Gagal sync data admin:", e); 
        } finally { 
            setFetching(false); 
        }
    };
  ...
  const handleApplyVoucher = async () => {
    ...
    } catch (e) { setVoucherError("Gagal cek voucher"); }
                ...
                <div className="space-y-3 font-bold uppercase tracking-widest">
                    ...
                    <div className="pt-6 border-t border-slate-50 flex justify-between items-end">
                        <div className="space-y-1">
                            ...
                            <h2 className="text-3xl font-black text-slate-900 tracking-tighter tabular-nums leading-none italic uppercase"><span className="text-sm not-italic text-blue-600 mr-1">Rp</span>{finalPrice.toLocaleString('id-ID')}</h2>
                        </div>
                    </div>
                </div>
            </div>
